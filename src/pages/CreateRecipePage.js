...
function CreateRecipePage() {
  ...
  const [listIngredients, setListIngredients] = useState([{quantite : "", nomIngr : ""}]);
    ...
    <div id='createPage'>
      ...
      <div id='listInstructions'>
        {instructions.map((instruction, index) => (
          <div>
            <input placeholder={'Instruction ' + index} onChange={(e) => handleChangeIns(e, index)} value={instructions[index]} />
            {index ? 
              <button onClick={() => removeInstruction(index)}>-</button> : 
              <button onClick={() => setInstructions([...instructions, ""])} >+</button>}
          </div>
        ))}
      </div>
      ...
      <div id='listIngredients'>
        {listIngredients.map((ingredient, index) => (
          <div>
            <input name="quantite" type="text" placeholder='Quantité' onChange={(e) => handleChangeIngr(e, index)} value={listIngredients[index].quantite || ""} />
            <input name="nomIngr" type="text"  placeholder='Ingrédient' onChange={(e) => handleChangeIngr(e, index)} value={listIngredients[index].nomIngr || ""}/>
            {index ?
              <button onClick={() => removeIngr(index)}>-</button> :
              <button onClick={() => setListIngredients([...listIngredients, {quantité : "", nomIngr : ""}])}>+</button>}
          </div>
        ))}
      </div>
    </div>
